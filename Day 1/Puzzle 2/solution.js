...
if (process.argv[2] == '--help') {
    console.log(`please pass a file, delimiter, either duo or trio, and the desired sum`)
...
function solutionTrios(arr) {
    ...
    arr.map(itemOne => {
        arr.map(itemTwo => {
            arr.map(itemThree => {
                if (itemOne !== itemTwo && itemOne !== itemThree && itemTwo !== itemThree && itemOne + itemTwo + itemThree == desiredSum) {
...
fs.readFile(file, 'utf8', function (err, data) {
    if (err) throw err;
    let dataArr = data.split(delimiter).sort((a, b) => a - b).slice(1).map(
        function (x) {
